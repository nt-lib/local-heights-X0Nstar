...
function find_and_test_model_2(model_map, known_rat_pts : primes_to_test := [], max_prime := 50, printlevel := 1)
  ...
  // The preimage operator @@ requires the scheme to be a subscheme of the codomain.
  // We first intersect the image curve with the line at infinity.
  infinity_scheme_on_image := image_curve meet Scheme(projective_plane, [z]);
  scheme_at_infinity_z := infinity_scheme_on_image @@ model_map;
