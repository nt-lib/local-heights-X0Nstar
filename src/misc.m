...
function coefficients_mod_pN(fake_rat_pts, rat_pts, divisors, base_pt, splitting_indices, data : printlevel := 0)
  ...
  M := (Matrix(pAdicField(data`p, data`N), data`g, data`g, [basis_integrals[i,j] : i,j in [1..data`g]]))^-1;
  ...
  rat_coeffs  := [Eltseq(ChangeRing(index_matrix*M*Matrix(data`g,1,Eltseq(ints)[1..data`g]), Integers())) : ints in rat_integrals];
  ...
  fake_coeffs := [Eltseq(ChangeRing(index_matrix*M*Matrix(data`g,1,Eltseq(ints)[1..data`g]), Integers())) : ints in fake_integrals];
